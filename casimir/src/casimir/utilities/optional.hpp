...
namespace Casimir::utilities {
    ...
    private:
        typedef std::bool_constant<
                std::is_move_constructible<T>::value && std::is_copy_constructible<T>::value> _isTCopyMove;
    ...
    public:
        ...
        inline T* getPtr() const {
            ...
            if (isEmpty())
                CASIMIR_THROW_EXCEPTION("NoSuchValueException", "No value was present in the given optional");
#endif
        ...
        inline T* orElsePtr(T* replacement) const {
            return (isEmpty()) ? (replacement) : (getPtr());
        }
        ...
        Optional<U> map(std::function<U(T)> mapper) {
            return isPresent() ? Optional<U>::of(mapper(get())) : Optional<U>::empty();
        }
